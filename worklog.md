...
Task ID: 13
Agent: main (Super Z)
Task: "প্রাইস ও ক্যান্ডেল Quotex-এর সাথে ১০০% মিলছে না + চার্ট খুললে ~৯০ ক্যান্ডেল" — রুট-কজ ৪টি ধরে সম্পূর্ণ সমাধান

Work Log:
- রুট-কজ নির্ণয় (লাইভ টোকেন j9JD… দিয়ে ৭টি প্রোব):
  ① চার্টের হিস্ট্রি ছিল Yahoo রিয়েল-মার্কেট — Quotex OTC ফিডের সাথে কখনোই মিলত না (EURUSD=X ≠ EURUSD_otc)
  ② Quotex-এর আসল হিস্ট্রি আসে history/list/v2 ইভেন্টে {"candles":[[time,open,close,high,low,ticks,realTime]…~২০০টি]} — আগের সেশন এই ইভেন্ট পুরো স্কিপ করত (raw-tick ভেবে) — তাই প্রতি পেয়ারে মাত্র ১০-২০টি ক্যান্ডেল পেত (ইউজারের "~৯০" সমস্যা)
  ③ INSERT OR IGNORE — Yahoo-র ভুল ক্যান্ডেল একই মিনিটে Quotex-এর সঠিকটাকে ব্লক করত
  ④ Quotex OTC ফিড-জেনারেশন পর্যায়ক্রমে রোটেট হয় — পুরনো সংযোগ পুরনো জেনারেশনে আটকে থাকে (প্রমাণ: ৭ পেয়ারে হুবহু মিল + EURUSD-এ ৩০ মিনিট ~৭ পিপ ডাইভারজেন্স; রোটেশন-টেস্টে দীর্ঘ সংযোগ কাটা পড়া ধরা)
- প্রোটোকল-আবিষ্কার (প্রোব দিয়ে প্রমাণিত): history/load-এর রেসপন্স ২ রকম — history/load ইভেন্ট (dict-ক্যান্ডেল ~১২টি, `time` প্যারামিটার দিয়ে পেজ-ব্যাক) + history/list/v2 (~২০০ ক্যান্ডেল, প্রথম রিকোয়েস্টে); ক্যান্ডেল-ফরম্যাট [t,o,c,h,l,tk,realTime] — invariant-টেস্টে ১৯৬/১৯৬ যাচাই; demo/real ফিড একই (ক্যান্ডেল হুবহু মিল) কিন্তু ইউজার অ্যাকাউন্ট-টাইপ বাছাই যোগ করা হলো
- ফিক্স (backend):
  - quotex_client.py: history/list/v2-এর .candles পার্স (tk টিক-কাউন্টসহ), র-টিক .history স্কিপ, garbage-guard (close≤0), request_history(base, end_time_s) পেজিং-মেথড, is_demo প্যারামিটার
  - market_engine.py: on_quotex_candles ফুল-রিপ্লেস (প্রথম পেলোডে db.delete_pair_candles + স্টোর রিসেট — Yahoo-দূষণ চিরতরে বাদ), নতুন-পেলোড-সর্বদা-বিজয়ী মার্জ (জেনারেশন-রোটেশন সেলফ-হিলিং) + db.upsert_candles
  - _deep_history_loop: `time`-প্যারামিটার পেজিং — প্রতি পেয়ারে ২০০→৭২০ ক্যান্ডেল (~১২ ঘণ্টা), ১.৫s পেসিং, ৩ খালি-পেজে স্টপ
  - _canary_loop + _canary_check: প্রতি ৩ মিনিটে ফ্রেশ ক্যানারি-সংযোগ — ① প্রি-স্ন্যাপশট ক্যান্ডেল-মিলান ② লাইভ-টিক মিলান (ক্রসটক-ইমিউন); ডাইভারজেন্স ধরলে অটো-রোল + ২৫ মিনিট পরপর প্রোঅ্যাক্টিভ রোল (Quotex-অ্যাপের মতো বর্তমান জেনারেশনে থাকা)
  - db.py: accountMode কলাম মাইগ্রেশন, delete_pair_candles, upsert_candles; get_history ডিফল্ট ৫০০
  - main.py: get-history ক্যাপ ১০০০, connect-token-এ ঐচ্ছিক isDemo, save-settings accountMode
- ফিক্স (frontend): settings-tab-এ অ্যাকাউন্ট-টাইপ সিলেক্টর (ডেমো/রিয়েল — connect-token-এ isDemo পাঠায়, লাইভ থাকলে পুনঃসংযোগ), ফিড-লেবেলে "লাইভ Quotex (টিক, ডেমো/রিয়েল)", চার্টে ৫০০ ক্যান্ডেল রিকোয়েস্ট + Quotex-অ্যাপের মতো ওপেনিং-ভিউ (~১২০ দৃশ্যমান, পেয়ার-বদলে সেট, রিফেচে ইউজার-জুম অক্ষত)
- ভেরিফিকেশন (সব লাইভ, ব্যবহারকারীর বৈধ টোকেনে):
  - ক্যান্ডেল: ৮/৮ পেয়ারে ১০০% হুবহু মিল র-Quotex হিস্ট্রির সাথে (EURUSD ১৯৯+১৯৯, USDJPY/AUDUSD/GBPUSD/EURJPY/GBPJPY/USDCAD ১৯৮×৬, USDCHF ১৯৯ — মোট ১৫৮৬/১৫৮৬ OHLC বিট-এক্সাক্ট)
  - লাইভ টিক: ইঞ্জিন বনাম ফ্রেশ-প্রোব Δ=0.00002 (একই দাম হুবহু)
  - ক্যানারি সাইকেল লাইভ-রান: কোনো ডাইভারজেন্স নেই, প্রতি ৩ মিনিটে ক্রসটক-রিফাইনে শেষ ~৬ মিনিটের ক্যান্ডেল ক্যানোনিক্যাল হয়ে যায়
  - ব্রাউজার: চার্ট রেন্ডার + অ্যানিমেশন সক্রিয় (পিক্সেল-ডিফ), ৩টি পেয়ার-সুইচ ত্রুটিহীন, ০ console-error, অ্যাকাউন্ট-সিলেক্টর দৃশ্যমান
  - ব্যাকটেস্ট (১০০% Quotex ডেটা): ২০৯১ ক্যান্ডেল → ৮ সিগন্যাল @ ৬৩% WR; eslint ✓ tsc ✓
- সিক্রেট-স্ক্যান: staged diff ক্লিন; scripts/ gitignored (টোকেন-ধারী প্রোব-ফাইল)

Stage Summary:
- চার্ট এখন ১০০% Quotex ডেটা: হিস্ট্রি ২০০→৭২০ ক্যান্ডেল/পেয়ার (Quotex-নিজস্ব), লাইভ টিক হুবহু, প্রতি ৩ মিনিটে সেলফ-হিলিং + জেনারেশন-রোটেশনে অটো-রোল
- ইউজারের Quotex-অ্যাপের সাথে মিলতে হলে: সেটিংসে অ্যাকাউন্ট-টাইপ (ডেমো/রিয়েল) ঠিক রাখুন — ব্যালেন্স দেখে বোঝা যায় কোনটা (ডেমো ৯১৯১.৩২, রিয়েল ০)
- User action: Railway রিডিপ্লয় → টোকন সংযোগ থাকলেই নতুন কোড সক্রিয়; "● লাইভ Quotex (টিক, ডেমো/রিয়েল)" লেবেল নিশ্চিত করুন
